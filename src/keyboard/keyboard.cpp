...
namespace red
{
	...
	bool Rkeyboard::isKeyPressed(int key)
	{
        SDL_Event event;
        ...
        if (SDL_PollEvent(&event))
        {
            switch( event.type )
            {
                case SDL_KEYDOWN:
                   ...
                   break;
                case SDL_KEYUP:
                   this->keyboard[event.key.keysym.sym] = false;
                   break;
            }
        }
        if(keyboard[key])
	...
	int l_Rkeyboard_constructor(lua_State * l)
		{
			//const char * name = luaL_checkstring(l, 1);

			Rkeyboard ** udata = (Rkeyboard **) lua_newuserdata(l, sizeof(Rkeyboard *));
			*udata = new Rkeyboard();

			luaL_getmetatable(l, "luaL_Rkeyboard");
		...
		Rkeyboard * l_CheckRkeyboard(lua_State * l, int n)
		{
			return *(Rkeyboard **)luaL_checkudata(l, n, "luaL_Rkeyboard");
		}

		int l_Rkeyboard_isKeyPressed(lua_State * l)
		{
			Rkeyboard * key= l_CheckRkeyboard(l, 1);
			int k = luaL_checknumber(l, 2);
			lua_pushboolean(l,key->isKeyPressed(k));
			return 1;
		}
	

		int l_Rkeyboard_destructor(lua_State * l)
		{
			Rkeyboard * foo = l_CheckRkeyboard(l, 1);
			delete foo;

			return 0;
		}

		void RegisterRkeyboard(lua_State *l)
		{
			luaL_Reg sRkeyboardRegs[] =
			{
				{ "new", l_Rkeyboard_constructor },
				{ "isKeyPressed", l_Rkeyboard_isKeyPressed },
				{ "__gc", l_Rkeyboard_destructor },
				{ NULL, NULL }
			};

			luaL_newmetatable(l, "luaL_Rkeyboard");

			//luaL_register(l, NULL, sRkeyboardRegs); --> VERSION <= 5.1
			luaL_setfuncs(l,sRkeyboardRegs,0);
			...
			lua_setglobal(l, "Rkeyboard");

            add_enum_to_lua( l, "Rkeys"
